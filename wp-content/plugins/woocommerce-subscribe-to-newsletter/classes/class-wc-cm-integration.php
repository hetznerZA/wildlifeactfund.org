	...
	public function show_stats() {
		...
		if ( ! $stats = get_transient( 'woocommerce_cmonitor_stats' ) ) {
			...
			if ( $result->was_successful() ) {
				...
				$stats .= '<li><strong style="font-size:3em;">' . $result->response->TotalActiveSubscribers . '</strong> ' . __( 'Total subscribers', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '<li><strong style="font-size:3em;">' . $result->response->NewActiveSubscribersToday . '</strong> ' . __( 'Subscribers today', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '<li><strong style="font-size:3em;">' . $result->response->NewActiveSubscribersThisMonth . '</strong> ' . __( 'Subscribers this month', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '<li><strong style="font-size:3em;">' . $result->response->UnsubscribesThisMonth . '</strong> ' . __( 'Unsubscribes this month', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '</ul>';
