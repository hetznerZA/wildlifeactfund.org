	...
	if ( ! class_exists( 'WC_Subscribe_To_Newsletter' ) ) {
		...
		class WC_Subscribe_To_Newsletter {
			...
			function init_form_fields() {
				...
				$mailchimp_lists = array( '' => __( 'Enter your key and save to see your lists', 'wc_subscribe_to_newsletter' ) );

				if ( $mailchimp->has_api_key() ) {
					if ( is_array( $mailchimp->get_lists() ) ) {
						$mailchimp_lists = array_merge( array( '' => __( 'Select a list...', 'wc_subscribe_to_newsletter' ) ), $mailchimp->get_lists() );
					} else {
						$mailchimp_lists = array( '' => __( 'Please create a list in your MailChimp account', 'wc_subscribe_to_newsletter' ) );
					}
				}
